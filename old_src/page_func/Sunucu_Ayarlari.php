...
class oServerDetails {
	...
	function degiskenbul($satirbasi,$kackarakter,$dosya) {
		$this->icerik = '';
		$ac = @fopen ($dosya, 'r+') or die ("Dosya açılamadı Hata Kodu : oynucaz-237wds");
		while (!feof($ac)) {
			$satir = @fgets ($ac); 
...
if(isset($_POST["yardir"]) && file_exists($oGCP['server.cfg']['dosya'])) {
	...
	$ssh2->SFTP_DownloadFile($serverinfo["ServerPath"].'/cstrike/server.cfg',"tmp/".$serverinfo["ServerIP"]."_".$serverinfo["ServerPort"]."_".(int)$_SESSION["OGCP_UserID"]."_server.cfg");
	$dosya = "tmp/".$serverinfo["ServerIP"]."_".$serverinfo["ServerPort"]."_".(int)$_SESSION["OGCP_UserID"]."_server.cfg";
	$dt = fopen($dosya, 'w');
	fwrite($dt, $oGCP['dosya']['icerik']);
	fclose($dt);
	if( $ssh2->SFTP_UploadFile("tmp/".$serverinfo["ServerIP"]."_".$serverinfo["ServerPort"]."_".(int)$_SESSION["OGCP_UserID"]."_server.cfg",$serverinfo["ServerPath"].'/cstrike/server.cfg') ) {
