...
function Adm_SendTicketMessage($ID,$Message,$Status) {
	...
	($Status == 1 && $Message == "") || $Status == 2  ? $query2 = "UPDATE ogcp_tickets SET TicketStatus = '".intval($Status)."' WHERE TicketID = '".intval($ID)."'" : $query2 = "";
...
function Adm_AddUser($Email,$Email2,$Comment,$Pass,$Name,$City,$Address,$Telephone,$Group,$Machine,$Servers,$Users,$Announ,$Tickets,$Plugins,$Files) {
	...
	$sql_query = "INSERT INTO `ogcp_users` (`UserEmail`, `UserPassword`, `UserEmail2` , `UserComment`,`UserName`, `UserCity`, `UserAddress`, `UserTelephone`, `UserGroup`, `UserLastLogin`, `ShowMachine`, `ShowServers`, `ShowUsers`, `ShowAnnouncements`, `ShowTickets`, `ShowPlugins`, `ShowFiles`) VALUES ('".addslashes($Email)."','".md5(md5($Pass))."', '".addslashes($Email2)."', '".addslashes($Comment)."', '".addslashes($Name)."', '".addslashes($City)."', '".addslashes($Address)."', '".addslashes($Telephone)."', '".(int)$Group."', 0, '".(int)$Machine."','".(int)$Servers."','".(int)$Users."','".(int)$Announ."','".(int)$Tickets."','".(int)$Plugins."','".(int)$Files."')";
	$sql_query = $baglan->query($sql_query);
	if($sql_query == false) return 0; else return $baglan->lastInsertId();
}

function Adm_UpdateUser($ID,$Email,$emailstatus,$Password,$email2,$comment,$Name,$City,$Address,$Telephone,$Group,$Machine,$Servers,$Users,$Announ,$Tickets,$Plugins,$Files,$Prefix) {
	if( (int)$ID <= 0 || $Email == "" || $Name == "" || $City == "" || $Address == "" || $Telephone == "" || (int)$Group <= 0 || (int)$Group > 3 ) return false;
	global $baglan;
	$sql_query = "UPDATE ogcp_users SET UserEmail = '".addslashes($Email)."', UserName = '".addslashes($Name)."',UserPassword = '".$Password."', UserEmail2 = '".addslashes($email2)."', UserComment = '".addslashes($comment)."', UserCity = '".addslashes($City)."', UserAddress = '".addslashes($Address)."', UserTelephone = '".addslashes($Telephone)."', UserGroup = '".(int)$Group."', UserPrefix = '".addslashes($Prefix)."', ShowMachine = '".(int)$Machine."', ShowServers = '".(int)$Servers."', ShowUsers = '".(int)$Users."', ShowAnnouncements = '".(int)$Announ."', ShowTickets = '".(int)$Tickets."', ShowPlugins = '".(int)$Plugins."', ShowFiles = '".(int)$Files."' WHERE UserID = '".(int)$ID."'";
...
function SendTicket($Title,$Message,$priority) {
	if( $Title == "" && $Message == "" && (int)$priority == 0) return false;
	global $baglan;
	$sql_query = $baglan->query("INSERT INTO `ogcp_tickets` (`TicketUserID`, `TicketTitle`, `TicketServerID`, `TicketMessageID`, `TicketStatus`, `TicketPriority`) VALUES ('".$_SESSION["OGCP_UserID"]."','".$Title."','".$_SESSION["OGCP_SelectedServer"]."','0','0','".$priority."');");
